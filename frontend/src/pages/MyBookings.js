  ...
  const fetchBookings = async () => {
    try {
      if (user.role === 'customer') {
        ...
        // Salon owner sees bookings for their salon
  ...
  if (loading) return (
    <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ color: '#6b7280', fontSize: '18px', marginTop: '16px' }}>Loading bookings...</p>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb', padding: '40px 32px' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          <p style={{ color: '#db2777', fontWeight: '600', letterSpacing: '2px', textTransform: 'uppercase', fontSize: '13px' }}>
            {user.role === 'salon_owner' ? 'Booking Requests' : 'Your Appointments'}
          </p>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: '40px', color: '#1f2937', marginBottom: '8px' }}>
            {user.role === 'salon_owner' ? '📋 Salon Bookings' : '📅 My Bookings'}
          </h1>
          <p style={{ color: '#6b7280' }}>
            {user.role === 'salon_owner'
              ? `${bookings.filter(b => b.status === 'pending').length} pending requests need your attention`
        ...
        {filtered.length === 0 ? (
          <div style={{
            ...
            <p style={{ color: '#6b7280', marginTop: '8px', marginBottom: '24px' }}>
              {user.role === 'salon_owner'
                ? 'No booking requests yet. Share your salon to get bookings!'
                ...
                <div key={booking._id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                    <div>
                      ...
                      {user.role === 'salon_owner' ? (
                        <>
                          <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: '22px', color: '#1f2937' }}>
                  ...
                  <div style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px solid #f3f4f6', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    {user.role === 'salon_owner' ? (
                      // Salon owner can confirm or cancel
                      <>
                        {booking.status === 'pending' && (
                          <>
                            <button onClick={() => handleUpdateBooking(booking._id, 'confirmed')} style={{
                              padding: '10px 24px', background: '#f0fdf4',
                              border: '2px solid #86efac', color: '#16a34a',
                              borderRadius: '25px', cursor: 'pointer',
                              fontWeight: '700', fontSize: '14px'
                            }}>✓ Confirm Booking</button>
                            <button onClick={() => handleUpdateBooking(booking._id, 'cancelled')} style={{
                        ...
                        {booking.status !== 'pending' && (
                          <p style={{ color: '#9ca3af', fontSize: '14px', padding: '10px 0' }}>
                            {booking.status === 'confirmed' ? '✅ You confirmed this booking' : '❌ You cancelled this booking'}
                          </p>
                        )}
                      </>
                    ) : (
                      // Customer can view salon or cancel pending booking
                      <>
                        <button onClick={() => navigate(`/salon/${booking.salon?._id}`)} style={{
                        ...
                        {booking.status === 'pending' && (
                          <button onClick={() => cancelBooking(booking._id)} style={{
                            padding: '10px 24px', background: '#fef2f2',
                            border: '2px solid #fca5a5', color: '#dc2626',
                            borderRadius: '25px', cursor: 'pointer',
                            fontWeight: '700', fontSize: '14px'
